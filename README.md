...
1. First load opens the **Learning Plan tour**: spotlight on the Kanban, each column (**Know → Apply**), then **task card** areas (competency, source, XP, call-to-action). **Skip** or **Next** through **Go to my Learning Plan**.
2. **Learning Plan** (after tour): time-aware greeting, streak, level bar. Open a card **CTA**, then **Mark Complete** on the placeholder activity.
...
6. **?** (top right) can restart the **Learning Plan** tour (and other tours). **Exit tour** (link, X button, or Escape) dismisses anytime; first visit exit also marks the tour as seen.
...
1280px width is ideal; layout scrolls horizontally on the Learning Plan and timeline down to ~768px.
